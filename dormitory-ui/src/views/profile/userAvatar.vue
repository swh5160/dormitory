...
import { updateSelf } from '@/api/student'
  ...
  methods: {
    ...
    beforeUpload (file) {
      if (file.type.indexOf('image/') === -1) {
        this.msgNoticeError('文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。')
      } else {
    ...
    uploadImg () {
      this.$refs.cropper.getCropBlob(async file => {
        ...
        let user = getInfoRes.data
        let formData = new FormData()
        formData.append('file', file, this.fileName)
        const uploadRes = await upload(formData)
        await updateSelf({ studentId: user.id, avatar: uploadRes.data })
        this.open = false
        this.options.img = uploadRes.data
        this.visible = false
